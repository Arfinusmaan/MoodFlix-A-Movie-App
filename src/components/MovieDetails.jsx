...
const MovieDetails = () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(
          movieTitle
        )}+official+movie+trailer&type=video&key=${YOUTUBE_API_KEY}`
      );
      const data = await response.json();

      if (data?.items?.length > 0 && data.items[0]?.id?.videoId) {
        setTrailerId(data.items[0].id.videoId);
        localStorage.setItem(movieTitle, data.items[0].id.videoId);
      } else {
        setTrailerId(null);
      }
    } catch (error) {
  ...
  return (
    <main className="bg-[#1A1A2E]">
      <div className="py-20">
        <div className="text-white wrapper px-15 movie-card max-w-[1200px] mx-auto shadow-[#362f3e] shadow-[0px_0px_48px_3px] bg-[#0b0b1d]">
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-center lg:items-start">
            <img
              src={movie.Poster}
              className="w-[230px] rounded-xl object-cover h-[340px]"
            ...
            <div className="space-y-6 max-lg:space-y-4">
              ...
              <div className="flex flex-col lg:flex-row justify-between lg:items-center">
                <div className="flex gap-4 lg:gap-6">
                  <strong className="min-w-[150px]">Genres:</strong>
                  <div className="flex flex-wrap gap-4 lg:gap-6">
                    {movie.Genre.split(", ").map((genre, index) => (
                      <span
                        className="border-none rounded-sm bg-[#3c3c5c] px-6 py-1 lg:px-8"
                        key={index}
                      >
                        {genre}
                      </span>
                ...
                <button
                  className="mt-4 lg:mt-0 ml-auto block cursor-pointer border-none p-4 bg-gradient text-sm rounded-sm font-semibold text-black"
                  onClick={() => navigate("/")}
                >
                  Visit Homepage →
                </button>
              ...
              <div className="flex flex-col lg:flex-row lg:items-center lg:gap-6">
                ...
                <p className="max-w-lg max-lg:max-w-md">{movie.Plot}</p>
              </div>
